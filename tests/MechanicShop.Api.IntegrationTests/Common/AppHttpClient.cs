...
/*
  without it In every test:
    Create token
    Set Authorization header
    Serialize/deserialize JSON
    Handle errors
    That’s noise, not testing.
 
 A test-friendly wrapper around HttpClient
*/
public class AppHttpClient(HttpClient httpClient)
{
    public async Task<string> GenerateTokenAsync(AppUser user)
    {
        ...
        var response = await httpClient.PostAsJsonAsync("identity/token/generate", generateTokenQuery);
    ...
    public async Task<T?> PostAndGetFromJsonAsync<TRequest, T>(string requestUri, TRequest value,
        CancellationToken cancellationToken = default)
    {
